...
# TODO list (중요도)
 - 화재 불만 잡는 model 만들기 ****
 - 다른 건물에도 화재가 날 수 있으므로, 화재 증강 데이터 만들어보기 **
 - 영상 간 앙상블 시스템 구축하기 -> 특히 시설물 ****
 - 프레임 2개 보기(영상에서 비정상 프레임이 추출될 경우 영상 폐기 우려) **


전송 주소: http://192.168.250.2:8080/SHUUFT9A

접속 주소: http://192.168.250.2:8080/main/team
...
5. **시설물 6슬롯 강제 매핑**: 시설물 위치가 고정되어 있다는 사전 지식을 이용해,
   ...
   "시설물 누락" 감점을 원천 차단합니다.
6. **Claude API + 오프라인 폴백**: JSON 결과 전송(6단계) 자체가 인터넷 연결을 필요로
   하므로 보고서도 온라인 API(Claude Haiku)를 우선 시도하되, 실패하면(키 미설정/네트워크
...
`report_generator.py`는 기본적으로 Claude API(Haiku 모델)를 사용해 상황보고서를
생성합니다. 키가 없으면 자동으로 오프라인 템플릿으로 폴백되므로 임무 자체는
실패하지 않지만, LLM 문장을 쓰려면 아래대로 API 키를 설정해야 합니다.

### 1) API 키 발급
https://console.anthropic.com 에서 로그인 후 API 키를 발급받고 크레딧을 충전합니다.

### 2) 환경변수 설정
코드에 키를 직접 적지 말고 `ANTHROPIC_API_KEY` 환경변수로 설정합니다.
- **Windows(PowerShell)**: `$env:ANTHROPIC_API_KEY = "발급받은키"` (세션 한정) 또는
  `setx ANTHROPIC_API_KEY "발급받은키"` (영구 등록, 이후 새로 여는 터미널부터 적용)
- **macOS/Linux**: `export ANTHROPIC_API_KEY="발급받은키"`

대회 PC에서 재부팅 후에도 유지하려면 `setx`(또는 시스템 환경변수 GUI)로 등록해두세요.

### 3) 정상 동작 확인
`python scripts/run_mission.py --synthetic`을 실행했을 때 결과 출력의
"보고서(claude)"처럼 `claude`로 표시되면 실제 API가 쓰인 것이고,
...
- [ ] 대회 현장 인터넷 연결과 `ANTHROPIC_API_KEY` 환경변수(+ 크레딧 잔액)가 대회 PC에
      설정되어 있는지 **사전 확인** — 네트워크가 끊기면 자동으로 오프라인 템플릿 폴백으로 동작함
