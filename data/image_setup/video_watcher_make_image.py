...
사용법:
  1) 폴더를 계속 감시하며 새 영상이 들어올 때마다 자동 처리:
     python scripts/video_watcher.py --watch

  2) 폴더에 이미 있는 영상들만 한 번 처리하고 종료:
...
class _VideoCreatedHandler:
    ...
    def __init__(self, args, FileSystemEventHandlerBase):
        self.args = args
        ...
        class Handler(FileSystemEventHandlerBase):
            def on_created(handler_self, event):
                ...
                if not event.src_path.lower().endswith(fc.VIDEO_FILE_EXTENSIONS):
                    return
                try:
                    process_video(event.src_path, self.args)
...
def run_once(args):
    """폴더에 이미 존재하는 영상 파일들을 한 번 처리하고 종료"""
    ...
    if not files:
        ...
        return
    for f in files:
        process_video(os.path.join(video_dir, f), args)


def run_watch(args):
    """watchdog으로 폴더를 계속 감시하며 새 영상이 생기면 자동 처리"""
    try:
    ...
    handler = _VideoCreatedHandler(args, FileSystemEventHandler).handler_instance
...
def main():
    ...
    parser.add_argument("--video-dir", type=str, default=VIDEO_INPUT_DIR, help="감시할 영상 입력 폴더")
    parser.add_argument("--output", type=str, default=VIDEO_OUTPUT_DIR, help="결과 이미지 저장 폴더")
