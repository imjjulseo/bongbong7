...
DETECTOR_BACKEND = "classical"   # "classical" | "yolo" - 폭파구/불발탄 통합 탐지 (확정 파이프라인: yolo)
...
# 2026-07-14: 1차 학습을 yolov8n(Roboflow 2장 라벨링, annotation/data.yaml)으로 진행함.
# 그 data.yaml은 클래스명을 알파벳순으로 정렬해서 내보내므로 이 순서를 그대로 따라야 함
# (0:big,1:cluster,2:dumb,3:medium,4:missile,5:small) - 기존에 가정했던 순서와 다름.
# 나중에 다른 학습 데이터셋을 쓸 경우 반드시 그 data.yaml의 names 순서와 다시 맞출 것.
YOLO_OBJECT_WEIGHTS = "models/yolov8n_object.pt"
...
# ---------------------------------------------------------
DASHBOARD_ENDPOINT_URL = "http://192.168.250.2:8080/main"  # 대회 측 전송 주소 (mission_code는 JSON 본문에 포함)
